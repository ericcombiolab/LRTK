...
def moduleFQ(args):	
	...
	if((args.input_type == "10x") & (args.output_type == "ULRF")): #convert 10x to ULRF
		conversion.TENx2ULRF(args.input_fastq1, args.input_fastq2, args.output_fastq1, args.output_fastq2, args.barcodes, args.filter, args.sort, args.threads)
	elif((args.input_type == "stLFR") & (args.output_type == "ULRF")): # convert stLFR to ULRF
		conversion.stLFR2ULRF(args.input_fastq1, args.input_fastq2, args.output_fastq1, args.output_fastq2, args.barcodes, args.filter, args.sort, args.threads)
	elif((args.input_type == "TELLSeq") & (args.output_type == "ULRF")): # convert TELL-Seq to ULRF
		conversion.TELLSeq2ULRF(args.input_fastq1, args.input_fastq2, args.index_fastq, args.output_fastq1, args.output_fastq2, args.filter, args.sort, args.threads)
	elif((args.input_type == "stLFR") & (args.output_type == "10x")): # convert stLFR to 10x
		conversion.stLFR210x(args.input_fastq1, args.input_fastq2, args.output_fastq1, args.output_fastq2)
	elif((args.input_type == "TELLSeq") & (args.output_type == "10x")): # convert TELL-Seq to 10x 
		conversion.TELLSeq210x(args.input_fastq1, args.input_fastq2, args.index_fastq, args.output_fastq1, args.output_fastq2, args.barcodes)
...
def moduleBAM(args):
	if((args.platform == "10x")):
		alignment.align_10x(args.input_barcoded_fastq1, args.input_barcoded_fastq2, args.input_fastq1, args.input_fastq2, args.read_group, args.reference, args.outfile, args.sort, args.mark_duplication, args.threads)
	elif(args.platform == "stLFR"):
		alignment.align_stLFR(args.input_barcoded_fastq1, args.input_barcoded_fastq2, args.input_fastq1, args.input_fastq2, args.read_group, args.reference, args.outfile, args.sort, args.mark_duplication, args.threads)
	elif(args.platform == "TELLSeq"):
		alignment.align_TELLSeq(args.input_barcoded_fastq1, args.input_barcoded_fastq2, args.read_group, args.reference, args.outfile, args.sort, args.mark_duplication, args.threads)
	else:
...
def modulePHASE(args):
	...
	if((args.application == "HapCUT2")):
		...
		phasing.Phasing_SpecHap(args.bam, args.vcf, args.reference, args.threads, args.outfile)
	else:
	...
	while(line):
		if line.startswith("#"):
			...
			if((tech == "10x")):
				...
				conversion.TENx2ULRF(input_fastq1, input_fastq2, outfq1, outfq2, whitelist, "Yes", "Yes", threads)
				alignment.align_10x(wbfq1, wbfq2, wobfq1, wobfq2, rg, ref, bamfile, "Yes", "Yes", threads)
				
			elif(tech == "stLFR"):
				whitelist=args.database + "/WhiteList/white_list_stlfr_barcode.fa"
				conversion.stLFR2ULRF(input_fastq1, input_fastq2, outfq1, outfq2, whitelist, "Yes", "Yes", threads)
				alignment.align_stLFR(wbfq1, wbfq2, wobfq1, wobfq2, rg, ref, bamfile, "Yes", "Yes", threads)

			elif(tech == "TELLSeq"):
				conversion.TELLSeq2ULRF(input_fastq1, input_fastq2, index_fastq, outfq1, outfq2, "Yes", "Yes", threads)
				alignment.align_TELLSeq(wbfq1, wbfq2, rg, ref, bamfile, "Yes", "Yes", threads)
