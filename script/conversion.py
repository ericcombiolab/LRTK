...
def stLFR2ULRF(infq1, infq2, outfq1, outfq2, white_list, filt, sort, threads, bin):
	...
	run_cmd(
		[
		"python",
		bin+"/script/extract_bc_stLFR.py",
		infq2, 
		bc_fq],
		"Extract_barcode"
	)
...
def TELLSeq2ULRF(infq1, infq2, idxfq, outfq1, outfq2, filt, sort, threads, bin):
	...
	run_cmd(
		[
		"python", 
		bin + "/script/extract_bc_TELLSeq.py", 
		idxfq, 
		tmpbq
		],
		"extract_barcode"
	...
	run_cmd(
		[
		"python", 
		bin + "/script/correct_bc.TELLSeq.py", 
		"-F1", infq1, 
		"-F2", infq2, 
		"-B", idxfq, 
		"-O1", tmpfq1, 
...
def stLFR210x(infq1, infq2, outfq1, outfq2, bin):
	...
	#Format conversion
	subprocess.call(["sh", bin+"/script/extract.bq.stLFR.sh", infq1, tmpbq])
	subprocess.call(["perl", bin+"/script/merge.stLFR.barcodes.pl", tmpbq, bin + "/SCRIPTS/4M-with-alts-february-2016.10x.txt", tmpmap, "8"])
	subprocess.call(["perl", bin+"/script/stLFR.fake.10x.pl", infq1, infq2, tmpmap, outfq1, outfq2, tmpidx])	
...
def TELLSeq210x(infq1, infq2, inxfq, outfq1, outfq2, white_list, bin):
	...
	#Format conversion
	subprocess.call([bin+"/src/ust10x", "-i1", inxfq, "-r1", infq1, "-r2", infq2, "-wl", white_list], cwd=outdir)
	subprocess.call(["mv", tmpfq1, outfq1])
	subprocess.call(["mv", tmpfq1, outfq1])
