...
def calling_freebayes(bam, ref, threads, outfile, bin):
	###########################################################################
	...
	run_cmd(
		[
		"python", 
		bin+"/script/fasta_generate_regions.py",
		ref,
		"100000",
		tmp_region
		],
	...
	run_cmd(
		[
		"perl",
		bin + "/script/Filt.SmallVariants.pl",
		tmp_vcf,
		outfile
		],
		"filteration"
...
def calling_samtools(bam, ref, threads, outfile, bin):
	###########################################################################
	...
	subprocess.call(cmd, shell=True)
	subprocess.call(["perl", bin + "/script/Filt.SmallVariants.pl", "", "-i", tmp_vcf, "-o", outfile])

	
def calling_GATK(bam, ref, threads, outfile, parameter):
	###########################################################################
